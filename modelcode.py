...
model = RNN(input_size, num_layers, hidden_size, num_classes, embedding_dim, vocab_size)
model.to(device)
print(model)
...
#     corpus = Counter(word_list)
#     # sorting on the basis of most common words
#     corpus_ = sorted(corpus,key=corpus.get,reverse=True)[:1000] # type: ignore
#     # creating a dict
#     onehot_dict = {w:i+1 for i,w in enumerate(corpus_)}
...
#         # print(output.squeeze().size())
#         # print(labels.size())
#         if (labels.size() != torch.Size([50])):
#             break
#         train_loss = criterion(output.squeeze(), labels.float()) #LOSS
#         # print("current loss: ", train_loss)
#         train_loss.backward()
#         train_accuracy = acc(output, labels) #ACCURACY
...
# print("\n ---------------------------------------- \n" + "AVG TRAIN ACC: ", (sum(train_acc_list) / len(train_acc_list)), "\n", "AVG VALID ACC: ", (sum(valid_acc_list) / len(valid_acc_list)))
